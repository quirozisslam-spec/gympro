...
def registrar_asistencia():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO asistencias(usuario_id,registrado_por,observacion) VALUES(%s,%s,%s)",
                (usuario_id,session["user_id"],data.get("observacion")))
        conn.commit()
        ...
        with conn.cursor() as cur:
            ...
            if existing:
                ...
                cur.execute("INSERT INTO asistencias(usuario_id,fecha,dia_semana,registrado_por) VALUES(%s,%s,%s,%s)",
                    (uid,fecha_dia,dia,session["user_id"]))
                conn.commit()
